      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="glass-panel p-6 border border-white/10 space-y-4">
            ...
            {acceptedApps.length === 0 ? (
              <div className="text-center py-6">
                <p className="text-xs text-slate-400 mb-3">You have not joined an active department yet.</p>
                <a href="/discover">
                  <Button variant="glow" size="sm">Browse Startups Hiring Builders</Button>
                </a>
              </div>
            ) : (
              <div className="space-y-3">
                {acceptedApps.map(app => (
                  <div key={app._id} className="p-4 rounded-2xl bg-white/[0.02] border border-white/5 flex items-center justify-between">
                    <div>
                      <div className="font-bold text-sm text-white">{app.departmentName}</div>
                      <div className="text-xs text-slate-400 mt-0.5">Startup: <b className="text-indigo-400">{app.startupName}</b></div>
                      <div className="text-[10px] text-emerald-400 font-medium mt-1">✓ Active Builder Membership</div>
                    </div>
          ...
          <div className="glass-panel p-6 border border-white/10 space-y-4">
            ...
            {tasks.length === 0 ? (
              <p className="text-xs text-slate-400 py-6 text-center">No tasks available in your department.</p>
            ) : (
              <div className="space-y-3">
                {tasks.map(task => {
                    ...
                    <div
                      ...
                      className={`p-4 rounded-2xl border transition-all ${
                        isDone
                          ...
                          : 'bg-white/[0.02] border-white/5 hover:border-sky-500/30'
                      }`}
                    >
                            ...
                            <>
                              {task.status === 'backlog' && (
                                <button
                                  ...
                                  className="px-2.5 py-1 rounded text-[11px] bg-sky-500/10 text-sky-300 hover:bg-sky-500/20 border border-sky-500/20 font-semibold"
                                >
                                  Claim & Start
                                </button>
                              )}
                              {task.status === 'in_progress' && (
                                <button
                                  ...
                                  className="px-2.5 py-1 rounded text-[11px] bg-purple-500/10 text-purple-300 hover:bg-purple-500/20 border border-purple-500/20 font-semibold"
                                >
                                  Submit Review
                                </button>
                              )}
                              {task.status === 'review' && (
                                <button
                                  ...
                                  className="px-2.5 py-1 rounded text-[11px] bg-emerald-500/10 text-emerald-300 hover:bg-emerald-500/20 border border-emerald-500/20 font-semibold"
                                >
                                  Mark Complete
                                </button>
                              )}
        ...
        <div className="space-y-6">
          ...
          <div className="glass-panel p-6 border border-white/10 space-y-4">
            ...
            <p className="text-xs text-slate-400">
              Founders review your portfolio before accepting department applications.
            </p>
            ...
            <div className="p-4 rounded-xl border border-dashed border-white/20 hover:border-indigo-500/50 bg-black/30 text-center relative cursor-pointer">
              ...
              <div className="text-xs font-semibold text-white">
                {isUploading ? 'Uploading...' : uploadSuccess ? 'Résumé Updated ✓' : 'Upload Résumé (PDF)'}
              </div>
            ...
            {resumeUrl && (
              <div className="text-xs text-indigo-400 flex items-center justify-between pt-1">
                ...
                <a href={resumeUrl} target="_blank" rel="noreferrer" className="underline">Preview File</a>
              </div>
            )}
          </div>
